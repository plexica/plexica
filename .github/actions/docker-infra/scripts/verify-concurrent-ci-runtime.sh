...
bootstrap() {
  local project="$1" runtime="${RUNNER_TEMP}/plexica-ci/$1" postgres_password storage_access_key storage_secret_key
  ...
  # storage container init, Keycloak DB attach, and host/container.env manifests
  ...
  storage_access_key=$(openssl rand -hex 24)
