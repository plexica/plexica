...
for (const [key, value] of [
  ['STORAGE_ACCESS_KEY', '00112233445566778899aabb'],
  ['STORAGE_SECRET_KEY', '00112233445566778899aabbccddeeff00112233445566778899aabbccddeeff'],
]) {
  if (!accepts(key, value)) throw new Error(`Rejected approved host credential scalar ${key}`);
}
if (accepts('STORAGE_SECRET_KEY', '')) {
  throw new Error('Accepted an empty storage secret key');
}
...
for (const [key, value] of [
  ...
  ['STORAGE_ENDPOINT', 'http://keycloak:9000'],
]) {
