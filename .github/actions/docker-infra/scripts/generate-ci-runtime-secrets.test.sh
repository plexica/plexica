...
for key in POSTGRES_PASSWORD STORAGE_ACCESS_KEY STORAGE_SECRET_KEY; do
...
for key in EVENT_KEY_ENCRYPTION_KEY PLUGIN_DB_ENCRYPTION_KEY PLUGIN_CREDENTIAL_PEPPER \
  POSTGRES_PASSWORD STORAGE_ACCESS_KEY STORAGE_SECRET_KEY; do
...
[[ "$(value_of "$full_out" STORAGE_ACCESS_KEY)" =~ ^[0-9a-f]{48}$ ]] ||
  { echo 'STORAGE_ACCESS_KEY is not a 24-byte hex value' >&2; exit 1; }
[[ "$(value_of "$full_out" STORAGE_SECRET_KEY)" =~ ^[0-9a-f]{64}$ ]] ||
  { echo 'STORAGE_SECRET_KEY is not a 32-byte hex value' >&2; exit 1; }
