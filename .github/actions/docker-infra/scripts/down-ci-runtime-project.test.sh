...
case "$*" in
  ...
  *'inspect'*'io.plexica.installation'*) printf 'plexica-ci-cleanup-123456 %s 123e4567-e89b-42d3-a456-426614174000\n' "$CI_PLUGIN_SCOPE" ;;
...
cat > "$temp/bin/docker" <<'EOF'
#!/usr/bin/env bash
 case "$*" in *'label=io.plexica.runtime-scope='*) [[ "$*" == *'ps -aq'* ]] && printf 'foreign\n' ;; *'inspect'*foreign*) printf 'plexica-ci-foreign-123456 %s 123e4567-e89b-42d3-a456-426614174000\n' "$CI_PLUGIN_SCOPE" ;; esac
EOF
chmod +x "$temp/bin/docker"
