...
describe('Plugin Concurrent Operations E2E Tests', () => {
  ...
  beforeAll(async () => {
    ...
    const tenant1Slug = `plugin-conc-1-${suffix}`;
    const tenant2Slug = `plugin-conc-2-${suffix}`;

    const createT1 = await app.inject({
      method: 'POST',
      url: '/api/tenants',
      headers: { authorization: `Bearer ${superAdminToken}` },
      payload: { slug: tenant1Slug, name: 'Plugin Concurrent Test Tenant 1' },
    });
    ...
    const createT2 = await app.inject({
      ...
      url: '/api/tenants',
      headers: { authorization: `Bearer ${superAdminToken}` },
      payload: { slug: tenant2Slug, name: 'Plugin Concurrent Test Tenant 2' },
    });
