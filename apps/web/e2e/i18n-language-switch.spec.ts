...
test.describe('E2E 006-07/08: i18n language switch', () => {
  ...
  test('EN→IT switch: full UI updates in place < 500ms, no reload, profile synced, date format changes', async ({
    ...
    try {
      ...
      // Role-scoped: the nav label also exists as a duplicate <span> inside the
      // mobile drawer (display:none); getByRole skips it, getByText would not.
      ...
      // Restore EN so later suites start from the default UI language — even on
      // failure: a leftover IT profile breaks every spec that hardcodes EN
      // strings (workspace create button, "Sign out", etc.).
      await setProfileLanguage(page, 'en');
    }
  });
  ...
  test('switch IT→EN back and forth stays consistent after reload (persisted)', async ({ page }) => {
    ...
    try {
      ...
      await page.reload();
      await expect(page.getByRole('link', { name: 'Registro di controllo', exact: true })).toBeVisible();
      expect(await readStoreLocale(page)).toBe('it');
