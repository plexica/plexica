  ...
  fastify.post('/api/v1/plugins/:installId/uninstall', { preHandler: [requireAbac('plugin:manage')] }, async (request) => {
    ...
    return withTenantDb(async (tx) => {
      ...
      await tx.actionRegistry.deleteMany({ where: { pluginId } }).catch((err: unknown) => {
        const msg = err instanceof Error ? err.message : String(err);
        logger.warn({ err: msg, installId, pluginId }, 'Failed to delete plugin action_registry entries during uninstall');
      });
      await tx.workspaceRoleAction.deleteMany({ where: { pluginId } }).catch((err: unknown) => {
        const msg = err instanceof Error ? err.message : String(err);
        logger.warn({ err: msg, installId, pluginId }, 'Failed to delete plugin workspace_role_action overrides during uninstall');
      });
      ...
      try {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        logger.warn({ err: msg, installId }, 'Plugin DB role drop failed during uninstall (best-effort)');
      }
      ...
      try {
        ...
        logger.warn({ err: (err as Error).message, installId }, 'Container removal failed during uninstall (best-effort)');
      }
      try {
