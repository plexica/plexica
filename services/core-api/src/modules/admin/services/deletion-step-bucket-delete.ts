...
// Deletion saga step handler: delete the tenant storage bucket (ADR-022 Decision 1).
...
/**
 * Force-deletes the storage bucket `tenant-<slug>` and all of its objects,
 * then removes the bucket. Idempotent — a missing bucket is treated as
 * success. Throws on storage service errors.
 */
