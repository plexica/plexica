...
// Unit tests for the lease-fenced settle (CodeRabbit #4): every settle must
// predicate on id AND lease_token, and a 0-row update (stale claim — row
// re-claimed after lease expiry or already purged by the GDPR deletion saga)
// is logged and ignored (returns false). Pure unit tests — Prisma
// emailQueue.updateMany mocked, logger mocked.
...
describe('settleEmailQueue — lease fencing (CodeRabbit #4)', () => {
  it('predicates every settle (sent/failed/dead) on id AND lease_token', async () => {
    ...
    expect(updateMany).toHaveBeenLastCalledWith(
      expect.objectContaining({ where: { id: 'row-1', leaseToken: 'tok-1' } })
    );
  ...
  it('returns false and ignores the update on a stale claim (0-row update)', async () => {
    ...
    expect(updateMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: { id: 'row-1', leaseToken: 'stale-tok' } })
    );
  });
});
