...
beforeAll(async () => {
  await requireInfra('tenant suspend integration tests');
  seeded = await provisionTenant({
    slug: SLUG,
    name: 'Suspend Test Org',
    adminEmail: `admin@${SLUG}.example`,
  });

  server = await createAdminTestServer([tenantSuspendRoutes]);
});
...
describe('POST /api/v1/admin/tenants/:id/suspend', () => {
  ...
  it('edge: tenant not found → 404', async () => {
    const res = await server.inject({
      method: 'POST',
      url: '/api/v1/admin/tenants/00000000-0000-0000-0000-000000000001/suspend',
      payload: { version: 1 },
    });
