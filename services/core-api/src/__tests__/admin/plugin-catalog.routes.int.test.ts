...
beforeAll(async () => {
  ...
  // adminPublishRoutes declares absolute /api/v1 paths, so it goes to rootRoutes.
  server = await createAdminTestServer([pluginCatalogRoutes], {
    rootRoutes: [adminPublishRoutes],
  });
});
...
describe('Plugin Catalog — review + publish/unpublish', () => {
  it('POST review approve — flips pending → approved (200)', async () => {
    ...
    const res = await server.inject({
      method: 'POST',
      url: '/api/v1/admin/plugins/test-cat-pending/review',
  ...
  it('POST review — rejects non-pending plugin (422 VALIDATION_ERROR)', async () => {
    ...
    const res = await server.inject({
      method: 'POST',
      url: '/api/v1/admin/plugins/test-cat-approved/review',
  ...
  it('POST review — 404 PLUGIN_NOT_FOUND for unknown slug', async () => {
    const res = await server.inject({
      method: 'POST',
      url: '/api/v1/admin/plugins/test-cat-missing/review',
      payload: { decision: 'approve' },
    });
  ...
  it('POST publish — 403 REVIEW_REQUIRED when not approved', async () => {
    ...
    const res = await server.inject({
      method: 'POST',
      url: '/api/v1/admin/plugins/test-cat-unapproved/publish',
    });
  ...
  it('POST publish — 200 after approval, then unpublish → unpublished (no installs)', async () => {
    ...
    const pub = await server.inject({
      method: 'POST',
      url: '/api/v1/admin/plugins/test-cat-flow/publish',
    });
    ...
    const unpub = await server.inject({
      method: 'POST',
      url: '/api/v1/admin/plugins/test-cat-flow/unpublish',
    });
  ...
  it('POST unpublish — → deprecated when installations exist', async () => {
    ...
    const unpub = await server.inject({
      method: 'POST',
      url: '/api/v1/admin/plugins/test-cat-dep/unpublish',
    });
    expect(unpub.statusCode).toBe(200);
    const body = JSON.parse(unpub.payload);
