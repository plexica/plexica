...
| 003-10 | Invite user via email                                          | Admin invites user, email arrives (Mailhog), user accepts, logs in |
...
2. A tenant admin can invite a user by email. The invite email is delivered
   (verified via Mailhog in tests), and the invited user can accept, log in,
...
| NFR-04 | User invitation email delivery (Mailhog)        | < 5s            |
...
| R-04 | Email delivery reliability in CI (Mailhog flakiness)              | LOW    | Retry with polling, increase Playwright timeout for email checks   |
