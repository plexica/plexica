# Tasks: 009 — Concurrent Self-Hosted CI Dynamic Ports
...
- [ ] **1.1** `[L]` `[CI-PORT-10]` `[CI-PORT-11]` `[P]` Create fail-closed runner admission and evidence capture.
  ...
  - **Implementation**: Before any install, build, pull, or Compose operation, require the runner-class marker, at least 4 online CPUs, at least 16 GiB effective cgroup memory, 12 GiB measured headroom, and 60 GiB Docker-root free space. Treat missing or unreadable evidence as failure. Write only non-secret evidence below the caller-provided private runtime directory and make the action expose this as an explicit preflight phase.
  ...
  - **Dependencies**: None.
  - **Verification**: `bash .github/actions/docker-infra/scripts/verify-ci-runner-capacity.test.sh`; exercise an absent marker and each threshold shortfall as non-zero cases.
...
- [ ] **1.2** `[L]` `[CI-PORT-01]` `[CI-PORT-02]` `[CI-PORT-03]` `[P]` Create the atomic dual-contract writer and source-direction checks.
...
- [ ] **1.3** `[M]` `[CI-PORT-02]` Add Docker/Compose/render preflight that proves CI overrides every host publication.
...
- [ ] **2.1** `[L]` `[CI-PORT-02]` `[CI-PORT-04]` `[CI-PORT-08]` Define the CI-only project-scoped Compose services and mount boundaries.
  - **Files**: Modify `docker-compose.ci.yml`; create `infra/compose/ci-runtime-services.yml`; modify `docker-compose.yml` only if an include is required for the CI overlay; modify `infra/compose/database-auth.yml`, `infra/compose/platform-services.yml`.
...
- [ ] **2.2** `[L]` `[CI-PORT-05]` `[P]` Gate Redpanda release on its inspected listener contract.
  - **Files**: Create `infra/redpanda/ci-entrypoint.sh`; modify `docker-compose.ci.yml`, `infra/compose/platform-services.yml`; create `.github/actions/docker-infra/scripts/redpanda-contract.test.sh`.
...
- [ ] **2.3** `[L]` `[CI-PORT-06]` `[CI-PORT-08]` `[P]` Stage Keycloak request-host discovery and reconcile only manifest-derived browser clients.
  - **Files**: Modify `infra/compose/database-auth.yml`, `infra/keycloak/reconcile-dev-keycloak.sh`, `infra/keycloak/reconcile-admin-client.sh`, `infra/keycloak/reconcile-tenant-clients.sh`; create `.github/actions/docker-infra/scripts/keycloak-contract.test.sh`.
...
- [ ] **3.1** `[L]` `[CI-PORT-03]` `[CI-PORT-04]` `[CI-PORT-06]` `[P]` Split Core public issuer validation from DNS-only Keycloak backchannels.
...
- [ ] **3.2** `[L]` `[CI-PORT-07]` `[P]` Implement a single project-scoped plugin container identity and runtime options.
...
- [ ] **3.3** `[L]` `[CI-PORT-07]` Enforce derived DNS-only plugin targets in proxy, recovery, and lifecycle paths.
...
- [ ] **4.1** `[L]` `[CI-PORT-03]` `[CI-PORT-12]` `[P]` Add mounted browser runtime configuration and strict same-origin endpoint parsing.
...
- [ ] **4.2** `[M]` `[CI-PORT-04]` `[CI-PORT-12]` Configure CI-only Vite server/preview proxy and host binding for both applications.
...
- [ ] **4.3** `[L]` `[CI-PORT-03]` `[CI-PORT-04]` `[CI-PORT-06]` Rewire CI Playwright and host provisioning consumers to the host manifest without starting local servers.
...
- [ ] **5.1** `[L]` `[CI-PORT-01]` `[CI-PORT-02]` `[CI-PORT-04]` `[CI-PORT-05]` `[CI-PORT-08]` Stage create/inspect/start/wait using only the appropriate contract.
  ...
  - **Dependencies**: Tasks 1.1–1.3, 2.1–2.3, 3.1, and 4.1–4.3.
...
- [ ] **5.3** `[L]` `[CI-PORT-09]` `[CI-PORT-10]` `[CI-PORT-11]` Rebuild the workflow as independently bootstrapped contract and CI jobs.
  ...
  - **Dependencies**: Tasks 1.1, 5.1, and 5.2.
...
- [ ] **5.4** `[L]` `[CI-PORT-01]` `[CI-PORT-05]` `[CI-PORT-07]` `[CI-PORT-09]` `[CI-PORT-11]` Implement the two-project full concurrent runtime verifier.
  ...
  - **Dependencies**: Tasks 2.1–2.3, 3.1–3.3, 4.1–4.3, 5.1–5.3.
...
- [ ] **6.2** `[S]` `[CI-PORT-01–12]` Conduct the required adversarial review and attach CI evidence.
  ...
  - **Verification**: `/forge-review .forge/specs/009-ci-dynamic-ports/`; confirm the completed workflow uploads non-secret admission and scoped diagnostic artifacts for both jobs.
...
| Document | Path |
| --- | --- |
| Tech spec | `.forge/specs/009-ci-dynamic-ports/tech-spec.md` |
| Plan | `.forge/specs/009-ci-dynamic-ports/plan.md` |
| ADR-031 | `.forge/knowledge/adr/adr-031-ci-runtime-contract-gated-orchestration.md` |
