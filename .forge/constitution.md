...
| September 2026 | Technology Stack (Object Storage row) | Object Storage row changed from `MinIO ^8` to a solution-agnostic neutral entry: digest-pinned `pgsty/silo` server image with the kept `minio` npm SDK (E2). | ADR-034 (amended 2026-09-16): server swap + neutral `storage`/`STORAGE_*` vocabulary; own identifiers never name a vendor; only E1 image ref, E2 npm SDK import, and E3 Compose `MINIO_ROOT_*` mapping targets (fed from `STORAGE_*`) may retain vendor strings. |
