...
**Last Updated**: March 7, 2026 (TD-013 added: NFR-005 content-hashed URLs deferred)
...
| TD-013     | NFR-005 content-hashed translation bundle URLs not implemented — `GET /api/v1/translations/:locale/:namespace` returns a plain `hash` field but the URL itself is not content-addressed. Serving bundles at stable URLs means CDN/browser caches cannot use immutable cache headers. Implementing content-hashed URLs requires URL routing changes and CDN invalidation strategy. Deferred pending CDN/deployment spec. | Performance | LOW        | Spec 006 review M-006, 2026-03-07        | Sprint 8                     |
