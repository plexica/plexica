...
| TD-011     | `requireRole` not exported from `vi.mock('../../middleware/auth.js')` in 5 unit test files — when RBAC was added to route `preHandler` options (Spec 007 HIGH #3), the auth mock in `core-services-flows.e2e.unit.test.ts`, `jobs.routes.unit.test.ts`, `notification.routes.unit.test.ts`, `search.routes.unit.test.ts`, and `storage.routes.unit.test.ts` did not include `requireRole`, causing 63 test failures. Fixed 2026-03-02. Future route unit tests must include `requireRole: vi.fn(() => vi.fn(...))` in all `vi.mock('../../middleware/auth.js')` calls. | Quality  | LOW      | Spec 007 review fix, 2026-03-02          | ✅ CLOSED 2026-03-02                                         |
