...
**Last Updated**: March 10, 2026 (Spec 014 TD-026–TD-039 all resolved; Spec 013 TD-025–TD-028 resolved)
...
| TD-021b    | `LayoutConfigService` is ~1,100 lines — exceeds Constitution Art. 4 single-responsibility guideline. Candidate split: `LayoutCrudService` (CRUD + soft-delete), `LayoutResolutionService` (resolveForUser + role logic), `LayoutCacheService` (Redis helpers). No functional risk currently; defer to a refactor sprint after Spec 014 stabilizes.                                                                                                                                                                                                                                                                                                                                                                                                                     | Maintainability | LOW               | forge-review 2026-03-08                         | Sprint 012                   |
...
| TD-022b    | `GET /layout-configs/forms` pagination is in-memory (slice after full manifest assembly). Acceptable for ≤100 forms but should be replaced with DB-level LIMIT/OFFSET via a pre-aggregated view if tenant plugin counts grow significantly.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Performance     | LOW               | forge-review 2026-03-08                         | Sprint 013                   |
| TD-023     | No automated performance benchmarks for `LayoutConfigService.resolveForUser()`. Constitution Art. 4.3 requires P95 < 200 ms; current Redis-cached path is expected to meet this but it has not been verified under load. Add k6 or Autocannon benchmark in the next performance sprint.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Performance     | LOW               | forge-review 2026-03-08                         | Sprint 012                   |
| TD-024     | `onRateLimited` callback duplicated 3x — The toast handler passed as `onRateLimited` to `TenantApiClient` and `AdminApiClient` is defined inline in `apps/web/src/lib/api-client.ts`, `apps/super-admin/src/lib/api-client.ts`, and `apps/web/src/hooks/useAuthorizationApi.ts`. All three share the same logic (parse `retryAfter`, format message, call `toast.warning` with dedup `id`). Fix: extract to a shared `createRateLimitToastHandler()` factory in `packages/api-client` or a shared `apps/web/src/lib/` utility. No correctness risk — only DRY violation. Deferred from Spec 016 forge-review finding M-01.                                                                                                                                              | Quality         | LOW               | forge-review 2026-03-10                         | Sprint 011                   |
| ~~TD-024b~~| ~~Spec 014 frontend components missing test coverage~~ — **RESOLVED in Pass 2**: 7 new test files with 100+ tests added. Residual gap (TD-032): US-007 save + warning dialog flow still untested.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Quality         | ~~HIGH~~ CLOSED   | forge-review-3 2026-03-08                       | Closed                       |
| ~~TD-025~~ | ✅ **RESOLVED 2026-03-10** — `page`/`pageSize` now threaded: `extensionRegistryService.superAdminListAllSlots(page, pageSize)` in controller; service signature updated to `superAdminListAllSlots(page = 1, pageSize = 50)`; repo already had correct `take`/`skip` logic.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Correctness     | ~~MEDIUM~~ CLOSED | forge-review 2026-03-10 (Spec 013 pre-existing) | Sprint 011 ✅                |
| ~~TD-025b~~| ~~`LayoutAwareForm` element-children mode section grouping~~ — **RESOLVED in Pass 2**: `childSectionMap` injected correctly via `React.cloneElement`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Quality         | ~~MEDIUM~~ CLOSED | forge-review-3 2026-03-08                       | Closed                       |
| ~~TD-026~~ | ✅ **RESOLVED 2026-03-10** — `isSuperAdmin()` now uses `.some(r => r === SYSTEM_ROLES.SUPER_ADMIN \|\| r.replace(/-/g, '_') === SYSTEM_ROLES.SUPER_ADMIN)` to accept both `'super_admin'` and `'super-admin'`. Two new unit tests added.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Security        | ~~MEDIUM~~ CLOSED | forge-review 2026-03-10 (Spec 013 pre-existing) | Sprint 011 ✅                |
| ~~TD-026b~~| ~~Stale `loadForms` `useCallback` closure~~ — **RESOLVED 2026-03-09**: Removed `selectedFormId` from `useCallback` deps; added `autoSelectedRef` guard; removed `eslint-disable-next-line` suppressor.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Maintainability | ~~MEDIUM~~ CLOSED | forge-review-3 2026-03-08                       | Closed                       |
| ~~TD-027~~ | ✅ **RESOLVED 2026-03-10** — Dead `else if ('type' in contribution …)` block removed from `ExtensionRegistryRepository.validateContributions()`. The `type_mismatch` branch was unreachable because `ExtensionContribution` has no `type` field. Logic collapsed to `if (!targetSlot) → target_not_found; else → valid`.                                                                                                                                                                                                                                                                                                                                                                                                                                                | Quality         | ~~LOW~~ CLOSED    | forge-review 2026-03-10 (Spec 013 pre-existing) | Sprint 011 ✅                |
| ~~TD-027b~~| ~~`buildSaveInput` not in `useCallback`~~ — **RESOLVED 2026-03-09**: Inlined `buildSaveInput` body into `handleSave` `useCallback`; removed `eslint-disable-next-line` suppressor.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Maintainability | ~~MEDIUM~~ CLOSED | forge-review-3 2026-03-08                       | Closed                       |
| ~~TD-028~~ | ✅ **RESOLVED 2026-03-10** — `extension-registry.routes.test.ts` happy-path test now asserts `toHaveBeenCalledWith(1, 50)`; custom-pagination test asserts `toHaveBeenCalledWith(2, 25)`. Both assertions confirm params are forwarded to service, not just echoed in response body.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Quality         | ~~LOW~~ CLOSED    | forge-review 2026-03-10 (Spec 013 pre-existing) | Sprint 011 ✅                |
| ~~TD-028b~~| ~~`savedConfig.updatedBy` renders as raw UUID~~ — **RESOLVED 2026-03-09**: Wrapped in `<span title={savedConfig.updatedBy}>` tooltip; test updated to match split-node text.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | UX              | ~~LOW~~ CLOSED    | forge-review-3 2026-03-08                       | Closed                       |
| ~~TD-029~~ | ~~`SectionWrapper` missing `label` prop~~ — **RESOLVED 2026-03-09**: Added `label={section.sectionId}` prop to `SectionWrapper` call in `LayoutAwareForm.tsx`; `fieldset` now gets `aria-labelledby`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Accessibility   | ~~LOW~~ CLOSED    | forge-review-3 2026-03-08                       | Closed                       |
| ~~TD-030~~ | ~~`detectRequiredFieldWarnings` false-positive on `readonly`~~ — **RESOLVED 2026-03-09**: Removed `\|\| globalVisibility === 'readonly'` from both `isGloballyHidden` and `hasRoleHidingAll` checks in `layout-config-validation.service.ts`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Correctness     | ~~MEDIUM~~ CLOSED | forge-review-3 2026-03-08                       | Closed                       |
| ~~TD-031~~ | ~~`buildManifestDefaults` uses array index as `order`~~ — **RESOLVED (prior session)**: Now uses manifest's declared `s.order` value.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Correctness     | ~~MEDIUM~~ CLOSED | forge-review-3 2026-03-08                       | Closed                       |
| ~~TD-032~~ | ~~US-007 save + warning dialog flow untested~~ — **RESOLVED 2026-03-09**: 6 new tests added to `LayoutConfigPanel.test.tsx`; covers dirty state, warning dialog trigger, proceed (with `acknowledgeWarnings: true`), cancel, and role-name display. 145 tests now passing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Quality         | ~~MEDIUM~~ CLOSED | forge-review-3 2026-03-08                       | Closed                       |
| ~~TD-033~~ | ~~`softDeleteByPlugin` over-fetches historical deleted rows~~ — **RESOLVED 2026-03-09**: Rewrote to single CTE with `RETURNING form_id, scope_type, scope_id` — only newly-deleted rows used for Redis invalidation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Performance     | ~~MEDIUM~~ CLOSED | forge-review-3 2026-03-08                       | Closed                       |
| ~~TD-034~~ | ~~ETag optimistic concurrency non-functional~~ — **RESOLVED 2026-03-09**: Added `etag: z.string().optional()` to `saveLayoutConfigSchema`; both PUT handlers now use `request.headers['if-match'] ?? body.etag ?? null`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Correctness     | ~~LOW~~ CLOSED    | forge-review-3 2026-03-08                       | Closed                       |
| ~~TD-035~~ | ~~`FieldConfigTable` row headers missing `scope="row"`~~ — **RESOLVED 2026-03-09**: Added `scope="row"` to both `<td role="rowheader">` elements in `FieldConfigTable.tsx`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Accessibility   | ~~LOW~~ CLOSED    | forge-review-3 2026-03-08                       | Closed                       |
| ~~TD-036~~ | ~~No React Query cache invalidation after save~~ — **RESOLVED 2026-03-09**: Added `useQueryClient()` + `queryClient.invalidateQueries({ queryKey: ['layout-engine', 'resolved', selectedFormId] })` after successful save in `LayoutConfigPanel.tsx`. Tests updated with `QueryClientProvider` wrapper.                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | UX              | ~~LOW~~ CLOSED    | forge-review-3 2026-03-08                       | Closed                       |
| ~~TD-037~~ | ~~Missing `return` after `reply.code(503).send()`~~ — **RESOLVED 2026-03-09**: Added `return;` after the send call in `layout-readonly-guard.ts` fail-closed catch block.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Correctness     | ~~LOW~~ CLOSED    | forge-review-3 2026-03-08                       | Closed                       |
| ~~TD-038~~ | ~~`validateSize()` not called from PUT route handlers~~ — **RESOLVED 2026-03-09**: Added `layoutConfigService.validateSize(body)` call (returns HTTP 413) to both tenant PUT and workspace PUT handlers in `layout-config.ts`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Correctness     | ~~LOW~~ CLOSED    | forge-review-3 2026-03-08                       | Closed                       |
| ~~TD-039~~ | ~~`warningItems` hardcodes `'affected roles'` string~~ — **RESOLVED 2026-03-09**: Replaced with computed role names — iterates `LAYOUT_ROLE_KEYS`, resolves per-role or global visibility in `draftFields`, joins hidden roles by name. TD-032 test verifies no `'affected roles'` literal in output.                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | UX              | ~~MEDIUM~~ CLOSED | forge-review-3 2026-03-08                       | Closed                       |
...
### Spec 014: Frontend Layout Engine — REVIEW PASS 3 COMPLETE (March 8, 2026)

**Date**: March 8, 2026  
**Context**: Tenant-configurable form and view layout system with per-role field visibility, ordering, and read-only controls

**Status**: ✅ spec done, ✅ plan done, ✅ implemented, ✅ review pass 1 resolved, ✅ review pass 2 resolved — 🚫 **review pass 3: 2 NEW HIGH findings blocking merge**

**Review Pass 3 Verdict**: NOT READY FOR MERGE  
**Review Artifact**: `.forge/reviews/014-frontend-layout-engine-review-3.md`  
**Blocking**: NEW-H-001 (PATCH vs PUT HTTP method mismatch — saves never succeed from frontend), NEW-H-002 (`ResolvedField` type missing `required`/`defaultValue` — FR-010 hidden field injection silently dead, test-spec coherence failure)

**Pass 2 → Pass 3 Resolutions**: H-001 (ARIA label test mismatch) ✅, H-002 (zero test coverage) ✅, M-003 (section grouping) ✅, M-005 (discriminated union) ✅
